...
@Component({
  selector: 'app-contacts-companies-table',
  standalone: true,
  imports: [AgGridAngular, TranslateModule],
  templateUrl: './contacts-companies-table.component.html',
  styleUrl: './contacts-companies-table.component.scss'
})
export class ContactsCompaniesTableComponent {
...
ngOnInit(): void {
  ...
  this.contactsService.getAllContacts().subscribe((data:Array<any>)=>{
    this.rowData = data.filter(person => person.contactType === contactType.Company).map(person => (person.contactType==contactType.Company ?
      {
      name: `${person.name}`,
...
{ field: "vatNumber", headerName: this.translateService.instant("VAT Number") },
...
private reloadContacts(type){
  ...
  this.contactsService.getAllContacts().subscribe((data:Array<any>)=>{
    this.rowData = data.map(person => (
      person.contactType==contactType.Company ?
      {
      name: `${person.name}`,
