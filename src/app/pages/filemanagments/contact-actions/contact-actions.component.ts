...
import { ContactsService, EncryptionService, ConfirmationDialogService, AlertService, FileManagmentService } from '@shared/services';
import { NgbModal } from '@ng-bootstrap/ng-bootstrap';
import { Subject, first } from 'rxjs';
import { ScreeningsListComponent } from '../../contacts/screenings-list/screenings-list.component';
import { File_Contacts } from '@shared/models';
import { NzGridModule } from 'ng-zorro-antd/grid';
...
@Component({
  ...
  imports: [CommonModule, NzGridModule, TranslateModule, NzToolTipModule],
   ...
   public encryptId(nId){
    ...
    public  deleteContact(id: string, content?): void {
      this.confirmationService.confirm('Are you sure you want to delete this?','This action cannot be undone. Deleting this contact will permanently remove it along with any screenings made.','Delete',"Cancel","lg").then((result) => {
        ...
        FileContact.filemanagmentid=this.rowData.filemanagmentid;
        FileContact.contactid=this.rowData.contactid;

          this.filemanagmentService.DeleteFileContact(this.rowData.filemanagmentid,FileContact.contactid)
          .pipe(first())
          .subscribe(() => 
            {             
      ...
      reloadComponent() {
        this.params.context?.loadLinkedContacts()
    }
    ...
    showModal(id): void {
      this.modalZService.create({
        ...
        nzData: {
          value: this.value,
          id: this.rowData.contactid,
          filemanagment_id: this.rowData.filemanagmentid,
        },
        nzFooter: []
      });
    }
