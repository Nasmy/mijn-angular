import { CUSTOM_ELEMENTS_SCHEMA, ChangeDetectionStrategy, ChangeDetectorRef, Component, EventEmitter, Input, NO_ERRORS_SCHEMA, OnInit, Output, SimpleChanges, ViewChild, inject } from '@angular/core';

import { File_Attachments, File_Folder, File_Managment, ResponseApi } from '@shared/models';
...
@Component({
  ...
  templateUrl: './file-attachments-modal.component.html',
  styleUrl: './file-attachments-modal.component.scss',

})
export class FileAttachmentsModalComponent {
  ...
  nzData: { value: number, fileFolders:File_Folder } = inject(NZ_MODAL_DATA);
  ...
  constructor(
    ...
    ngOnInit(): void {
     ...
     if (this.file_managmentId == null) {
      this.file_managmentId = this.nzData["value"];
     }
     ...
     if(this.folder!=null){
      this.file_managmentId=this.folder.fileManagmentId;
     }
      ...
      this.config1= <DropzoneConfigInterface> {
        ...
        accept: (file, done) => {
         this.filemanagmentService.createSingleFileAttachment(file,this._file_managmentId,this.folder!=null?this.folder.id:null)
         .subscribe({
         next: (data) => {
    ...
    if(this.folder==undefined && this.folder==null){
      this.filemanagmentService.loadAttachmentsByManagmentId(this._file_managmentId).subscribe((responseData:ResponseApi<Array<File_Attachments>>)=>{
        if (responseData.data!=null && responseData.data!=undefined){
          this.lstAttachments=responseData.data;
   ...
   loadAttachmentByFolder(){
    if(this.folder!=undefined && this.folder!=null){
      this.filemanagmentService.loadAttachmentsByFolderId(this.folder.id, this.folder.fileManagmentId).subscribe((responseData:ResponseApi<Array<File_Attachments>>)=>{
        if (responseData.data!=null && responseData.data!=undefined){
          this.lstAttachments=responseData.data;
          this.changeDetect.detectChanges;
        }
