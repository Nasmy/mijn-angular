...
@Component({
  selector: 'app-file-contacts-modal',
  standalone: true,
  imports: [
    NzCardModule,
    TranslateModule,
  ...
  templateUrl: './file-contacts-modal.component.html',
  styleUrl: './file-contacts-modal.component.scss'
})
export class FileContactsModalComponent {
        ...
        if(data!=null && data.length>0){
          this.rowData = data.map(FileContact=>{
            return {
              ...
              actions: FileContact.filemanagmentid+","+FileContact.contactid, 
    ...
    this.lstContacts$$.subscribe(data=>{
      if(data!=null && data.length>0){
        this.rowDataContacts = data.map(oContact=>{
          if(this._FileContacts$.value.length>0){
            let lstLinked=this._FileContacts$.value;
            let foundedLinkedContact=lstLinked.find(fc=>{return fc.contactid==oContact.contactId && fc.filemanagmentid==this.nzData['value']});
            return {
              name:oContact.name,
  ...
  checkLinkedContact(contactid:number):boolean{
    let lstLinked=this._FileContacts$.value;
    let foundedLinkedContact=lstLinked.find(fc=>{return fc.contactid==contactid&& fc.filemanagmentid==this.nzData['value']});
    return foundedLinkedContact==null && foundedLinkedContact==undefined;
  }


  loadLinkedContacts(){
    this.fileManagmentService.LoadAllContactsFilemanagment(this.nzData['value']).pipe(first()).subscribe({
  ...
  onFileAddContact(){
    ...
    selectedData.forEach(NewRole => {
      ...
      FileContact.filemanagmentid=this.nzData['value'];
      ...
      this.fileManagmentService.LinkContactToFile(this.nzData['value'],FileContact).pipe(first())
  ...
  private onFileRoleChange(RoleChange: File_Contacts){
    ...
    let FileContact:File_Contacts=<File_Contacts>{};
     ...
     this.fileManagmentService.ChangeRoleFileContact(this.nzData['value'],FileContact).pipe(first())
      .subscribe({next:(dataResponse)=>{
        this.loadLinkedContacts();
        this.loadAllContacts();
 ...
 /* onCellValueChange(rowData){
    ...
    loadContacts(){
      this.fileManagmentService.LoadAllContactsFilemanagment(this.nzData['value']).subscribe( data => {

        if(data!=null && data['data'].length > 0){
          this.rowData = data['data'].map(FileContact=>{
            return {
              name:FileContact.contact?.name,
              role:FileContact?.role,
              email:FileContact.contact?.email,
  ...
  loadFileContacts(){
    this.fileManagmentService.LoadAllContactsFilemanagment(this.nzData['value']).subscribe( data => {
      if(data!=null && data['data'].length>0){
        this.rowDataContacts = data['data'].map(oContact=>{
