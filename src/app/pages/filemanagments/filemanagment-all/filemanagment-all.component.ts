...
@Component({
  selector: 'app-filemanagment-all',
  standalone: true,
  imports: [AgGridAngular,TranslateModule],
  templateUrl: './filemanagment-all.component.html',
  styleUrls: ['./filemanagment-all.component.scss']
})
export class FilemanagmentAllComponent implements OnInit {
rowData;
...
public colDefs: ColDef<any>[] = [
  ...
  { field: "actions", headerName: this.translateService.instant("Actions"), cellRenderer: TableActionsComponent, floatingFilter: false, filter: false, width: 50}
  ];
...
private loadFilemanagmentsByCompany(){
  this.filemanagmentService.getByClient().subscribe();
}
